...
data "template_file" "tf-fastly-redirects-error" {
  template = file(
    "${path.module}/vcl/snippet/tf-fastly-redirects-error.vcl.tpl",
  )
...
data "template_file" "tf-fastly-redirects-recv" {
  template = file(
    "${path.module}/vcl/snippet/tf-fastly-redirects-recv.vcl.tpl",
  )

  vars = {
    rendered-redirects = indent(
      2,
      join(
        "\n",
...
data "template_file" "tf-path-fastly-redirect-partial" {
  template = file(
    "${path.module}/vcl/partial/tf-path-fastly-redirect-partial.vcl.tpl",
  )
